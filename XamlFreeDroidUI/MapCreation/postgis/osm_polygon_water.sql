 SELECT row_number() OVER () AS id,
  ...
  WHERE (planet_osm_polygon.water = ANY (ARRAY['canal'::text, 'basin'::text, 'ditch'::text, 'drainage_pond'::text, 'lake'::text, 'lagoon'::text, 'Irrigation_Canal'::text, 'intermittent'::text, 'lake;pond'::text, 'pond'::text, 'reservoir'::text, 'river'::text, 'stream'::text]));